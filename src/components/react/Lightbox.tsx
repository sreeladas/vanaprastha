  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-2 right-2 z-30 w-12 h-12 flex items-center justify-center rounded-full bg-foreground text-background hover:bg-foreground/90 text-2xl font-sans cursor-pointer transition-colors shadow-lg"
          aria-label="Close"
        >
          ✕
        </button>
        ...
        <div className="shrink-0 mt-4 pt-3 border-t border-warmgray-100 flex items-center justify-between gap-4">
          <div className="flex-1 min-w-0 text-center">
            {image.title && (
              <p className="text-foreground font-serif text-xl md:text-2xl line-clamp-2">
                {image.title}
              </p>
            )}
            {image.caption && (
              <p
                ref={captionRef}
                className="text-muted-foreground font-sans text-lg md:text-xl mt-1 line-clamp-2"
              >
                {image.caption}
              </p>
            )}
        ...
        {expanded && (
          ...
          >
            <div className="flex-1 min-h-0 overflow-y-auto">
              {image.title && (
                <p className="text-foreground font-serif text-2xl md:text-3xl">{image.title}</p>
              )}
              {image.caption && (
                <p className="text-muted-foreground font-sans text-lg md:text-xl mt-3 leading-relaxed">
                  {image.caption}
                </p>
              )}
            </div>
        ...
        {hasPrev && (
          <button
            ...
            className="absolute left-2 md:left-3 top-1/2 -translate-y-1/2 z-10 w-14 h-14 md:w-16 md:h-16 flex items-center justify-center rounded-full bg-foreground text-background hover:bg-foreground/90 text-3xl md:text-4xl cursor-pointer transition-colors shadow-lg"
        ...
        {hasNext && (
          <button
            onClick={() => onNavigate(currentIndex + 1)}
            className="absolute right-2 md:right-3 top-1/2 -translate-y-1/2 z-10 w-14 h-14 md:w-16 md:h-16 flex items-center justify-center rounded-full bg-foreground text-background hover:bg-foreground/90 text-3xl md:text-4xl cursor-pointer transition-colors shadow-lg"
